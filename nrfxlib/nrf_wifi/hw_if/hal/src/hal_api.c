...
static enum nrf_wifi_status hal_rpu_hard_rst(struct nrf_wifi_hal_dev_ctx *hal_dev_ctx,
                                             enum RPU_PROC_TYPE proc)
{
	...
	pr_err("clearing the ROM\n");
        /* Now clear the core memories */
        for (mem_type = HAL_RPU_MEM_TYPE_CORE_ROM;
             mem_type < HAL_RPU_MEM_TYPE_MAX;
             mem_type++) {
                status = hal_rpu_mem_clr(hal_dev_ctx,
                                         proc,
                                         mem_type);
                ...
                if (status != NRF_WIFI_STATUS_SUCCESS) {
                        nrf_wifi_osal_log_err(hal_dev_ctx->hpriv->opriv,
                                             "%s: Clearing of core mem (%d) failed\n",
                                             __func__,
                                             mem_type);
...
out:
        if (status == NRF_WIFI_STATUS_SUCCESS)
                nrf_wifi_osal_log_info(hal_dev_ctx->hpriv->opriv,
                                       "%s: Hard reset done for MCU%d\n",
                                       __func__,
                                       proc);
        else
                nrf_wifi_osal_log_err(hal_dev_ctx->hpriv->opriv,
                                     "%s: Hard reset failed for MCU%d\n",
                                     __func__,
                                     proc);
					  ...
					  unsigned int buf_id)
{
	...
	if (rx_buf_info->mapped) {
		...
		nrf_wifi_osal_log_err(hal_dev_ctx->hpriv->opriv,
				      "%s: Rx buffer not mapped for pool_id = %d, buf_id=%d\n",
				      __func__,
				      pool_id,
				      buf_id);
	}
				      ...
				      unsigned int buf_id)
{
	...
	nrf_wifi_osal_mem_cpy(hal_dev_ctx->hpriv->opriv,
                              (void *)bounce_buf_addr,
                              (void *)buf,
                              hal_dev_ctx->hpriv->cfg_params.rx_buf_headroom_sz);
#else
	rpu_addr = RPU_MEM_DATA_RAM_BASE + (bounce_buf_addr - hal_dev_ctx->addr_rpu_pktram_base);
...
#ifdef SOC_WEZEN
#ifdef INLINE_RX
	rx_buf_info->phy_addr = nrf_wifi_bal_dma_map_inline_rx(hal_dev_ctx->bal_dev_ctx,
						     addr_to_map,
						     buf_len,
					...
					unsigned int buf_id)
{
...
#ifdef SOC_WEZEN
#ifdef INLINE_RX
	unmapped_addr = nrf_wifi_bal_dma_unmap_inline_rx(hal_dev_ctx->bal_dev_ctx,
	...
	rpu_addr = RPU_MEM_DATA_RAM_BASE + (unmapped_addr - hal_dev_ctx->addr_rpu_pktram_base);
#endif
#else
...
static bool hal_rpu_soft_hpq_is_empty(struct nrf_wifi_hal_dev_ctx *hal_dev_ctx)
{
	...
	status = hal_rpu_mem_read(hal_dev_ctx,
        ...
        if (status != NRF_WIFI_STATUS_SUCCESS) {
                nrf_wifi_osal_log_err(hal_dev_ctx->hpriv->opriv,
                                      "%s: Read from the host_event_busy_index = %x address failed, val (0x%X)\n",
                                       __func__,
                                       &hal_dev_ctx->rpu_info.soft_hpq->host_cmd_free_index,
                                       current_index);
                return true;
					     ...
					     unsigned int msg_addr)
{
	...
	if (msg_type == NRF_WIFI_HAL_MSG_TYPE_CMD_CTRL) {
		status = hal_rpu_mem_read(hal_dev_ctx,
                ...
                if (status != NRF_WIFI_STATUS_SUCCESS) {
                        nrf_wifi_osal_log_err(hal_dev_ctx->hpriv->opriv,
                                              "%s: Read from the host_cmd_free_index = %x address failed, val (0x%X)\n",
                                              __func__,
                                              &hal_dev_ctx->rpu_info.soft_hpq->host_cmd_free_index,
                                              current_index);
                        goto out;
		...
		if (status != NRF_WIFI_STATUS_SUCCESS) {
			nrf_wifi_osal_log_err(hal_dev_ctx->hpriv->opriv,
					      "%s: Writing to cmd_busy_buffs[index=%d] address = %x failed msg_addr = %x\n",
					      __func__,
					      current_index,
					      &hal_dev_ctx->rpu_info.soft_hpq->cmd_busy_buffs[current_index],
					      msg_addr);
		...
		if (status != NRF_WIFI_STATUS_SUCCESS) {
			nrf_wifi_osal_log_err(hal_dev_ctx->hpriv->opriv,
					      "%s: Writing to host_cmd_free_index = %x failed for val =%d\n",
					      __func__,
					      &hal_dev_ctx->rpu_info.soft_hpq->host_cmd_free_index,
					      current_index);
			goto out;
						 ...
						 unsigned int *msg_addr)
{
	...
	if (msg_type == NRF_WIFI_HAL_MSG_TYPE_CMD_CTRL) {
		...
		if (status != NRF_WIFI_STATUS_SUCCESS) {
			nrf_wifi_osal_log_err(hal_dev_ctx->hpriv->opriv,
					      "%s: Read from the cmd_free_buffs[%d] = %x address failed, val (0x%X)\n",
					      __func__,
					      current_index,
					      &hal_dev_ctx->rpu_info.soft_hpq->cmd_free_buffs[current_index],
					      val);
	...
	if (!val) {
		nrf_wifi_osal_log_err(hal_dev_ctx->hpriv->opriv,
				      "%s:No valid content avaialble in the expected index \n",
				      __func__,
				      current_index);
		*msg_addr = 0;
		...
		if (status != NRF_WIFI_STATUS_SUCCESS) {
			nrf_wifi_osal_log_err(hal_dev_ctx->hpriv->opriv,
					      "%s: Write to address %xfailed, val (0x%X)\n",
					      __func__,
					      &hal_dev_ctx->rpu_info.soft_hpq->cmd_free_buffs[current_index],
					      0x0);
			return true;
						...
						unsigned int pool_id)
{
	...
	if (cmd_type == NRF_WIFI_HAL_MSG_TYPE_CMD_DATA_TX) {
		...
		/* Access the correponding index in tx_cmd_buffs with index same as current_index and Write addr to it*/
		status = hal_rpu_mem_write(hal_dev_ctx,
		...
		if (status != NRF_WIFI_STATUS_SUCCESS) {
			nrf_wifi_osal_log_err(hal_dev_ctx->hpriv->opriv,
					      "%s: Writing to tx_cmd_buffs[index=%d] address = %x failed val=%x\n",
					       __func__,
					       desc_id,
					       &hal_dev_ctx->rpu_info.soft_hpq->tx_cmd_buffs[current_index],
					       addr);
	...
	} else if (cmd_type == NRF_WIFI_HAL_MSG_TYPE_CMD_DATA_RX) {
		/* Post the updated information to the RPU */
		status = hal_rpu_msg_post(hal_dev_ctx,
					  cmd_type,
					  pool_id,
...
#ifdef RPU_HARD_RESET_SUPPORT
        for (proc = RPU_PROC_TYPE_MCU_LMAC;
             proc < RPU_PROC_TYPE_MAX;
             proc++) {
                /* Hard Reset the device */
                status = hal_rpu_hard_rst(hal_dev_ctx,
                                          proc);

                if (status != NRF_WIFI_STATUS_SUCCESS) {
                        nrf_wifi_osal_log_err(hpriv->opriv,
                                             "%s: hal_rpu_hard_rst failed for MCU %d\n",
                                             __func__,
                                             proc);
                        goto bal_dev_free;
                }
...
#ifdef SOFT_HPQM
	hal_dev_ctx->rpu_info.soft_hpq = (struct soft_hpqm_info *)HOST_RPU_GDRAM_START_ADDR;
					      ...
					      enum RPU_PROC_TYPE rpu_proc)
{
	...
	if (i == 1000) {
		nrf_wifi_osal_log_err(hal_dev_ctx->hpriv->opriv,
				      ...
				      "Expected: 0x%X, Actual: 0x%X address :0x%X\n",
				      __func__,
				      rpu_proc,
				      exp_val,
				      val,addr);
...
#ifdef SOC_WEZEN
#ifdef INLINE_RX
	hpriv->hostram_addr_base_inline_rx =  (unsigned long)nrf_wifi_osal_iomem_mmap_inline_rx(opriv,
                                                                        SOC_HOST_DATA_RAM_BASE,
                                                                        SOC_HOST_DATA_RAM_LEN);
	bal_cfg_params.addr_hostram_base_inline_rx = hpriv->hostram_addr_base_inline_rx;
#endif
#endif
...
#ifdef SOC_WEZEN
#ifdef INLINE_RX
	nrf_wifi_osal_iomem_unmap_inline_rx(hpriv->opriv,

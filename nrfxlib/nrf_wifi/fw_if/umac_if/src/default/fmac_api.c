...
#ifdef SOC_WEZEN
#ifdef CMD_RX_BUFF
	rx_buf_ipc = nrf_wifi_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv,
					      def_priv->num_rx_bufs * sizeof(struct nrf_wifi_rx_buf));
	...
	if (status != NRF_WIFI_STATUS_SUCCESS) {
		...
		goto out;
                ...
                nrf_wifi_osal_log_dbg(fmac_dev_ctx->fpriv->opriv,
                                      "%s During initialization UMAC rx buff" \
				       "programmed for num_buffs= %d\n", __func__, def_priv->num_rx_bufs);
                nrf_wifi_osal_mem_free(fmac_dev_ctx->fpriv->opriv,
                                       rx_buf_ipc);
        }
#endif /* CMD_RX_BUFF */
...
#define MAX_DEINIT_WAIT (1 * 1000 * 1000)
		nrf_wifi_osal_sleep_ms(fmac_dev_ctx->fpriv->opriv, 1);
		if (nrf_wifi_osal_time_elapsed_us(fmac_dev_ctx->fpriv->opriv,
				    ...
				    unsigned long address_offset)
{
	...
	enum nrf_wifi_status status = NRF_WIFI_STATUS_FAIL;
	fmac_dev_ctx = dev_ctx;
	//pr_err("%s-------address_offset=%lx key=%p key_len=%d----%d\n", __FUNCTION__, address_offset, key, key_len, __LINE__);
        status = hal_rpu_mem_write(fmac_dev_ctx->hal_dev_ctx,
                                   RPU_ADDR_SECURERAM_REGION + address_offset,
                                   key,
                                   key_len);
}
static void revMemCopy(uint8_t *dest, uint8_t *src, uint32_t size) {
        uint32_t byteCnt = size;
				 ...
				 unsigned char *peerDBIndex)
{
	...
	if (peer_key->key_type == NRF_WIFI_KEYTYPE_PAIRWISE) {
                for (i = 0; i < MAX_PEERS; i++) {
                        ...
                        if ((peerKeyDB + i)->keyValid == KEY_INVALID && (freeDB == 0xff))
                                freeDB = i;
                }
			     ...
			     unsigned char *peerDBIndex)
{
	...
	def_dev_ctx = wifi_dev_priv(fmac_dev_ctx);
	vif_ctx = def_dev_ctx->vif_ctx[if_idx];
	validDB = findValidPeerDBIndex(dev_ctx, peer_key, mac_addr, if_idx, peerDBIndex);
        if (validDB) {
                index = *peerDBIndex;
		peerKeyDB = &def_dev_ctx->peerKeyDB[index];
		if (peer_key->key_type == NRF_WIFI_KEYTYPE_PAIRWISE) { //UNICAST
                        switch (peer_key->cipher_suite) {
                        case NRF_WIFI_FMAC_CIPHER_SUITE_SMS4:
                                micKeyLen = PEER_MIC_KEY_LEN_16;
                                revMemCopy(peerKeyDB->ucstWapiMicKey, peer_key->key.nrf_wifi_key, PEER_MIC_KEY_LEN_16);
				...
				//print_hex_dump(KERN_ALERT, "nrf_wifi_key: ", DUMP_PREFIX_ADDRESS, 16, 1, peer_key->key.nrf_wifi_key, PEER_ENC_KEY_LEN_16, 1);
                        ...
                        case NRF_WIFI_FMAC_CIPHER_SUITE_SMS4:
                                micKeyLen = PEER_MIC_KEY_LEN_16 ;
                                revMemCopy(peerKeyDB->bcstWapiMicKey, peer_key->key.nrf_wifi_key, PEER_ENC_KEY_LEN_16);
					   ...
					   struct nrf_wifi_umac_key_info *key_info)
{
	...
	address_offset = (VIF_KEY_DB_OFFSET +
                   ...
                   VIF_MIC_KEY_LEN_PER_VIF + key_info->key_idx * (VIF_KEY_LEN_PER_KEYID));
	secure_crypto_reg_write(fmac_dev_ctx, key_info->key.nrf_wifi_key, key_info->key.nrf_wifi_key_len, 4, address_offset);
#endif
#endif
						...
						unsigned int rx_buf_nums)
{
	...
	rx_buf_iter = rx_buf;
	for (i = 0; i < counter; i++) {
		rx_buf_cmd = nrf_wifi_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv,

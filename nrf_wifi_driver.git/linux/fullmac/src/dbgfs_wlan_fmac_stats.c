...
static void nrf_wifi_lnx_wlan_fmac_dbgfs_stats_show_host(struct seq_file *m,
						      ...
						      struct rpu_host_stats *stats)
{
	...
	for (i = 0; i < fmac_dev_ctx->fpriv->data_config.max_tx_aggregation; i++) {
		cnt = stats->tx_coalesce_frames[i];
		if (cnt != 0)
			seq_printf(m,
				   "tx_coalesece_frames[%d] = %d\n",
				   (i + 1),
				   cnt);
	}
	...
	seq_printf(m,
		   ...
		   stats->num_isrs);
	seq_printf(m,
		   "total_event_pop_cnt = %u\n",
		   stats->num_events);
	seq_printf(m,
		   "total_event_resubmit_cnt_true = %u\n",
		   stats->num_events_resubmit);
	seq_printf(m,
		   "total_event_resubmit_cnt_false = %u\n",
...
static void nrf_wifi_lnx_wlan_fmac_dbgfs_stats_show_umac(struct seq_file *m,
						      struct rpu_umac_stats *stats)
{
	...
	seq_printf(m,
		   "tx_non_coalesce_pkts_rcvd_from_host = %d\n",
		   stats->tx_dbg_params.tx_non_coalesce_pkts_rcvd_from_host);

	seq_printf(m,
		   "tx_coalesce_pkts_rcvd_from_host = %d\n",
	...
	seq_printf(m,
		   "tx_cmds_currently_in_use = %d\n",
		   stats->tx_dbg_params.tx_cmds_currently_in_use);

	seq_printf(m,
		   "tx_done_events_send_to_host = %d\n",
		   stats->tx_dbg_params.tx_done_events_send_to_host);

	seq_printf(m,
		   "tx_done_success_pkts_to_host = %d\n",
		   stats->tx_dbg_params.tx_done_success_pkts_to_host);

	seq_printf(m,
		   "tx_done_failure_pkts_to_host = %d\n",
		   stats->tx_dbg_params.tx_done_failure_pkts_to_host);

	seq_printf(m,
		   "tx_cmds_with_crypto_pkts_rcvd_from_host = %d\n",
		   stats->tx_dbg_params.tx_cmds_with_crypto_pkts_rcvd_from_host);

	seq_printf(m,
		   "tx_cmds_with_non_cryptot_pkts_rcvd_from_host = %d\n",
		   stats->tx_dbg_params.tx_cmds_with_non_crypto_pkts_rcvd_from_host);

	seq_printf(m,
		   "tx_cmds_with_broadcast_pkts_rcvd_from_host = %d\n",
		   stats->tx_dbg_params.tx_cmds_with_broadcast_pkts_rcvd_from_host);

	seq_printf(m,
		   "tx_cmds_with_multicast_pkts_rcvd_from_host = %d\n",
		   stats->tx_dbg_params.tx_cmds_with_multicast_pkts_rcvd_from_host);

	seq_printf(m,
		   "tx_cmds_with_unicast_pkts_rcvd_from_host = %d\n",
	...
	seq_printf(m,
		   "rx_coalised_events = %d\n",
		   stats->rx_dbg_params.rx_coalesce_events);

	seq_printf(m,
		   "total_rx_pkts_from_lmac = %d\n",
	...
	seq_printf(m,
		   "null_skb_pointer_from_lmac = %d\n",
		   stats->rx_dbg_params.null_skb_pointer_from_lmac);

	seq_printf(m,
		   "unexpected_mgmt_pk = %d\n",
...
static void nrf_wifi_lnx_wlan_fmac_dbgfs_stats_show_lmac(struct seq_file *m,
						      struct rpu_lmac_stats *stats)
{
	...
	seq_printf(m,
		   "deaggInptrDescEmpty =%d\n",
		   stats->deagg_inptr_desc_empty);

	seq_printf(m,
		   "deaggCircularBufferFull =%d\n",
...
static void nrf_wifi_lnx_wlan_fmac_dbgfs_stats_show_phy(struct seq_file *m,
...
void show_display_scan_res(struct seq_file *m) {
	...
	for (i = 0; i < rpu_ctx_lnx->display_scan_params->num; i++) {
		...
		struct umac_display_results *res =
			rpu_ctx_lnx->display_scan_params->res[i];

		seq_printf(m, MACSTR "\t", MAC2STR(res->mac_addr));
		...
		switch (res->nwk_band) {
			...
			case NRF_WIFI_BAND_5GHZ:
				seq_printf(m, "%s\t", "5GHZ");
				break;
			case NRF_WIFI_BAND_60GHZ:
				seq_printf(m, "%s\t", "60GHZ");
				break;
			default:
				seq_printf(m, "%s\t", "<null>");
				break;
		...
		seq_printf(m,  "%d \t", res->nwk_channel);
		...
		seq_printf(m, "%d \t", res->beacon_interval);
		...
		if (res->ssid.nrf_wifi_ssid_len == 0)
			...
			seq_printf(m, "%s\n", res->ssid.nrf_wifi_ssid);
	}
}
#endif

static int nrf_wifi_lnx_wlan_fmac_dbgfs_stats_show(struct seq_file *m, void *v)
{
...
#ifdef notyet
#ifdef DEBUG_MODE_SUPPORT
	stats_type = rpu_ctx_lnx->conf_params.stats_type;
#endif /* DEBUG_MODE_SUPPORT */
#endif /* notyet */
...
#endif /* DEBUG_MODE_SUPPORT */
		nrf_wifi_lnx_wlan_fmac_dbgfs_stats_show_host(m,
							  rpu_ctx_lnx->rpu_ctx,
							  &stats->host);
...
#endif /* DEBUG_MODE_SUPPORT */
		nrf_wifi_lnx_wlan_fmac_dbgfs_stats_show_umac(m, &stats->fw.umac);

#ifdef DEBUG_MODE_SUPPORT
	if ((stats_type == RPU_STATS_TYPE_ALL) ||
	    (stats_type == RPU_STATS_TYPE_LMAC))
#endif /* DEBUG_MODE_SUPPORT */
		nrf_wifi_lnx_wlan_fmac_dbgfs_stats_show_lmac(m,
							  &stats->fw.lmac);
#endif /* !CONFIG_NRF700X_RADIO_TEST */
...
#endif /* DEBUG_MODE_SUPPORT */
		nrf_wifi_lnx_wlan_fmac_dbgfs_stats_show_phy(m,
#ifdef CONFIG_NRF700X_RADIO_TEST
							 op_mode,
#endif /* CONFIG_NRF700X_RADIO_TEST */
							 &stats->fw.phy);
	...
	if (rpu_ctx_lnx->connect_params.status == SCAN_RESULTS_COMPLETE) {
		if (rpu_ctx_lnx->display_scan_params) {
			if (rpu_ctx_lnx->display_scan_params->num > 0) {
				seq_puts(m, "\n************SCAN RESULTS*****************\n");
				seq_puts(m, "\nmac_addr / nwk_band / nwk_channel / protocol / security_type / beacon_interval / signal level / ssid\n");
				show_display_scan_res(m);
				seq_puts(m, "\n");
			}
		}
	}
     ...
     seq_printf(m, "twt_target_wake_interval_exponent  = %d\n",
             rpu_ctx_lnx->twt_params.twt_cmd.twt_target_wake_interval_exponent);
     seq_printf(m, "twt_target_wake_interval_mantissa  = %d\n",
             rpu_ctx_lnx->twt_params.twt_cmd.twt_target_wake_interval_mantissa);
	...
	switch(rpu_ctx_lnx->twt_params.teardown_reason) {
		...
		break;
		case 2:
			seq_printf(m, "twt_teardown_reason = DELAY_IN_TWT_UMAC_SLEEP_RESPONSE\n");
		break;
		case 3:
			seq_printf(m, "twt_teardown_reason  = INVALID_TWT_WAKE_INTERVAL\n");
		break;
		default:
		break;
	}
...
static int open_stats(struct inode *inode, struct file *file)
{
	...
	return single_open(file,
			   nrf_wifi_lnx_wlan_fmac_dbgfs_stats_show,
			   rpu_ctx_lnx);
}
					 ...
					 struct nrf_wifi_ctx_lnx *rpu_ctx_lnx)
{
	...
	rpu_ctx_lnx->dbgfs_wlan_stats_root = debugfs_create_file("stats",
								 0444,
								 root,
								 rpu_ctx_lnx,
...
fail:
	nrf_wifi_lnx_wlan_fmac_dbgfs_stats_deinit(rpu_ctx_lnx);

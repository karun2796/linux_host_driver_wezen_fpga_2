...
static int nrf_wifi_lnx_wlan_fmac_dbgfs_ver_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m,
		   "LMAC : %d.%d.%d.%d\n",
		   NRF_WIFI_LMAC_VER(fmac_dev_ctx->lmac_ver),
		   NRF_WIFI_LMAC_VER_MAJ(fmac_dev_ctx->lmac_ver),
		   NRF_WIFI_LMAC_VER_MIN(fmac_dev_ctx->lmac_ver),
		   NRF_WIFI_LMAC_VER_EXTRA(fmac_dev_ctx->lmac_ver));
#endif
...
static int open_ver(struct inode *inode, struct file *file)
{
	...
	return single_open(file,
			   nrf_wifi_lnx_wlan_fmac_dbgfs_ver_show,
			   rpu_ctx_lnx);
}

static const struct file_operations fops_ver = {
	.open = open_ver,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = NULL,
				       ...
				       struct nrf_wifi_ctx_lnx *rpu_ctx_lnx)
{
	...
	rpu_drv_priv.dbgfs_ver_root = debugfs_create_file("version",
							  0444,
							  root,
							  rpu_ctx_lnx,
							  &fops_ver);
...
fail:
	nrf_wifi_lnx_wlan_fmac_dbgfs_ver_deinit();

...
static ssize_t nrf_wifi_lnx_wlan_twt_write(struct file *file,
					...
					loff_t *ppos)
{
	...
	if (tok_s) {
		...
		if (tok_s) {
			...
			rpu_ctx_lnx->twt_params.twt_cmd.twt_target_wake_interval_exponent = val;
		}

		tok_s = strstr(conf_buf, " mantissa=");
		if (tok_s) {
			sscanf(tok_s + strlen(" mantissa="), "%lu", &val);
			rpu_ctx_lnx->twt_params.twt_cmd.twt_target_wake_interval_mantissa = val;
		}

		tok_s = strstr(conf_buf, " min_twt=");
		if (tok_s) {
			sscanf(tok_s + strlen(" min_twt="), "%lu", &val);
			rpu_ctx_lnx->twt_params.twt_cmd.nominal_min_twt_wake_duration = val;
		}
...
#define MAX_TWT_WAIT ( 1 * 1000 * 1000)
			if (osal_ops->time_elapsed_us(start_time_us) >= MAX_TWT_WAIT)
				break;
		}
...
static int nrf_wifi_lnx_wlan_fmac_dbgfs_twt_show(struct seq_file *m, void *v)
{
	struct nrf_wifi_ctx_lnx *rpu_ctx_lnx = NULL;
...
static int nrf_wifi_lnx_wlan_twt_open(struct inode *inode, struct file *file)
{

	struct nrf_wifi_ctx_lnx *rpu_ctx_lnx = (struct nrf_wifi_ctx_lnx *)inode->i_private;

	return single_open(file,
			   nrf_wifi_lnx_wlan_fmac_dbgfs_twt_show,
			   rpu_ctx_lnx);
}


static const struct file_operations fops_wlan_twt = {
	.open = nrf_wifi_lnx_wlan_twt_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = nrf_wifi_lnx_wlan_twt_write,
				       ...
				       struct nrf_wifi_ctx_lnx *rpu_ctx_lnx)
{
	...
	nrf_wifi_lnx_wlan_fmac_twt_init(&rpu_ctx_lnx->twt_params);
	...
	rpu_ctx_lnx->dbgfs_nrf_wifi_twt_root = debugfs_create_file("twt",
							       0644,
							       root,
							       rpu_ctx_lnx,
							       &fops_wlan_twt);
...
fail:
	nrf_wifi_lnx_wlan_fmac_dbgfs_twt_deinit(rpu_ctx_lnx);
out:
	return ret;
}

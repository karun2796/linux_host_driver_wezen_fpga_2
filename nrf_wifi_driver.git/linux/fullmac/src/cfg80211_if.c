...
int nrf_wifi_cfg80211_del_vif(struct wiphy *wiphy,
				struct wireless_dev *wdev)
{
	...
	nrf_wifi_fmac_vif_clear_ctx(rpu_ctx_lnx->rpu_ctx,
				      vif_ctx_lnx->if_idx);
	netdev = NULL;
				...
				struct vif_params *params)
{
	...
	if(!vif_ctx_lnx->event_set_if) {
		status = -ETIMEDOUT;
		pr_err("%s:Timed out waiting for response from RPU\n", __func__);
		goto out;
	}
...
int nrf_wifi_cfg80211_add_key(struct wiphy *wiphy,
				struct net_device *netdev,
				u8 key_index,
				bool pairwise,
				const u8 *mac_addr,
				struct key_params *params)
{
	...
	if (params->seq_len) {
		memcpy(key_info->seq.nrf_wifi_seq,
		       params->seq,
		       params->seq_len);
...
int nrf_wifi_cfg80211_del_key(struct wiphy *wiphy,
				struct net_device *netdev,
				u8 key_idx,
				bool pairwise,
				const u8 *mac_addr)
...
int nrf_wifi_cfg80211_start_ap(struct wiphy *wiphy,
				 struct net_device *netdev,
				 struct cfg80211_ap_settings *params)
{
	...
	start_ap_info->freq_params.frequency = params->chandef.chan->center_freq;
	start_ap_info->freq_params.channel_width = params->chandef.width;
	start_ap_info->freq_params.center_frequency1 = params->chandef.center_freq1;
	start_ap_info->freq_params.center_frequency2 = params->chandef.center_freq2;
	...
	if (params->chandef.width == NL80211_CHAN_WIDTH_20_NOHT)
		...
		start_ap_info->freq_params.channel_type = NRF_WIFI_CHAN_HT40PLUS;
	else if ((params->chandef.width == NL80211_CHAN_WIDTH_40) &&
		 (params->chandef.center_freq1 ==
		  params->chandef.chan->center_freq - 10))
		start_ap_info->freq_params.channel_type = NRF_WIFI_CHAN_HT40MINUS;

	if (params->ssid) {
		memcpy(start_ap_info->ssid.nrf_wifi_ssid,
		       params->ssid,
		       params->ssid_len);
	...
	if (params->beacon.head_len > 0) {
		memcpy(start_ap_info->beacon_data.head,
		       params->beacon.head,
	...
	if (params->beacon.tail_len > 0) {
		memcpy(start_ap_info->beacon_data.tail,
		       params->beacon.tail,
	...
	if (status == NRF_WIFI_STATUS_FAIL) {
		pr_err("%s: nrf_wifi_fmac_start_ap failed\n", __func__);
		goto out;
	}
...
int nrf_wifi_cfg80211_chg_bcn(struct wiphy *wiphy,
				struct net_device *netdev,
				struct cfg80211_beacon_data *params)
{
	...
	if (params->head_len > 0) {
		memcpy(bcn_info->beacon_data.head,
		       params->head,
		       params->head_len);
	...
	if (params->tail_len > 0) {
		memcpy(bcn_info->beacon_data.tail,
		       params->tail,
		       params->tail_len);
	...
	if (params->probe_resp_len > 0) {
		memcpy(bcn_info->beacon_data.probe_resp,
		       params->probe_resp,
...
int nrf_wifi_cfg80211_stop_ap(struct wiphy *wiphy,
				struct net_device *netdev)
{
	struct wireless_dev *wdev = NULL;
	struct nrf_wifi_ctx_lnx *rpu_ctx_lnx = NULL;
...
int nrf_wifi_cfg80211_add_sta(struct wiphy *wiphy,
				struct net_device *netdev,
				const u8 *mac,
				struct station_parameters *params)
{
...
#ifdef notyet
	flags2_info = &add_sta_info->sta_flags2;
	...
	if (params->ht_capa)
		memcpy(add_sta_info->ht_capability,
		       params->ht_capa,
...
#ifdef notyet
	status = nrf_wifi_fmac_add_sta(rpu_ctx_lnx->rpu_ctx,
					 vif_ctx_lnx->if_idx,
					 add_sta_info);
...
int nrf_wifi_cfg80211_del_sta(struct wiphy *wiphy,
				struct net_device *netdev,
				struct station_del_parameters *params)
{
	...
	if (params->mac)
		ether_addr_copy(del_sta_info->mac_addr,
				params->mac);
	...
	status = nrf_wifi_fmac_del_sta(rpu_ctx_lnx->rpu_ctx,
					 vif_ctx_lnx->if_idx,
					 del_sta_info);
...
int nrf_wifi_cfg80211_chg_sta(struct wiphy *wiphy,
				struct net_device *netdev,
				const u8 *mac,
				struct station_parameters *params)
{
	...
	if (params->supported_oper_classes_len > 0) {
		memcpy(chg_sta_info->supported_oper_classes.supported_oper_classes,
		       params->supported_oper_classes,
		       params->supported_oper_classes_len);
...
#ifdef CONFIG_NRF700X_AP_MODE
int nrf_wifi_cfg80211_chg_bss(struct wiphy *wiphy,
				struct net_device *netdev,
				struct bss_parameters *params)
{
	...
	status = nrf_wifi_fmac_set_bss(rpu_ctx_lnx->rpu_ctx,
					 vif_ctx_lnx->if_idx,
					 bss_info);
			     ...
			     struct cfg80211_scan_request *req)
{
	...
	scan_info->scan_params.scan_flags = req->flags;

	scan_info->scan_params.no_cck = req->no_cck;
	...
	if (req->ie_len) {
		scan_info->scan_params.ie.ie_len = req->ie_len;
		memcpy(scan_info->scan_params.ie.ie,
		       req->ie,
		       req->ie_len);
	}
#endif
	status = nrf_wifi_fmac_scan(rpu_ctx_lnx->rpu_ctx,
				      vif_ctx_lnx->if_idx,
				      scan_info);
...
void nrf_wifi_cfg80211_scan_start_callbk_fn(void *os_vif_ctx,
...
static void nrf_wifi_cfg80211_scan_results(struct net_device *dev,
					     struct nrf_wifi_umac_event_new_scan_results *new_scan_results)
{
	...
	if (new_scan_results->valid_fields &
		...
		ielen = new_scan_results->ies.ie_len;
		ftype = CFG80211_BSS_FTYPE_UNKNOWN;
	}
	if (new_scan_results->valid_fields &
	    NRF_WIFI_EVENT_NEW_SCAN_RESULTS_BEACON_IES_VALID) {
		ie = new_scan_results->beacon_ies.ie;
		ielen = new_scan_results->beacon_ies.ie_len;
	...
	res = cfg80211_inform_bss_width(wiphy,
					rx_channel,
					NL80211_BSS_CHAN_WIDTH_20,
					ftype,
					new_scan_results->mac_addr,
					new_scan_results->ies_tsf,
					new_scan_results->capability,
					new_scan_results->beacon_interval,
...
static void nrf_wifi_cfg80211_scan_done(struct nrf_wifi_fmac_vif_ctx_lnx *vif_ctx_lnx,
					  int aborted)
{
...
void nrf_wifi_cfg80211_rx_bcn_prb_rsp_callbk_fn(void *os_vif_ctx,
						  void *frm,
						  ...
						  short signal)
{
...
#ifdef notyet
		pr_err("%s: Recd invalid mgmt frame from RPU\n",
		       __func__);
#endif
		return;
	}
	...
	cfg80211_inform_bss_frame_data(rpu_ctx_lnx->wiphy,
				       &bss_meta,
				       mgmt,
				       len,
				       GFP_ATOMIC);
...
void nrf_wifi_cfg80211_scan_res_callbk_fn(void *os_vif_ctx,
					    ...
					    bool more_res)
{
	...
	nrf_wifi_cfg80211_scan_results(vif_ctx_lnx->netdev,
					 scan_res);

	if (!more_res)
		nrf_wifi_cfg80211_scan_done(vif_ctx_lnx,
					      false);
...
void nrf_wifi_cfg80211_scan_done_callbk_fn(void *os_vif_ctx,
...
void nrf_wifi_cfg80211_scan_abort_callbk_fn(void *os_vif_ctx,
					      struct nrf_wifi_umac_event_trigger_scan *scan_done_event,
					      unsigned int event_len)
{
	struct nrf_wifi_fmac_vif_ctx_lnx *vif_ctx_lnx = NULL;
...
int nrf_wifi_cfg80211_auth(struct wiphy *wiphy,
			     struct net_device *netdev,
			     struct cfg80211_auth_request *req)
{
	...
	memcpy(auth_info->nrf_wifi_bssid,
	       req->bss->bssid,
	       ETH_ALEN);
	...
	if (req->key_len) {
		...
		memcpy(auth_info->key_info.key.nrf_wifi_key,
		       req->key,
		       req->key_len);
		...
		if (req->key_len == 5)
			auth_info->key_info.cipher_suite = WLAN_CIPHER_SUITE_WEP40;
		else
			auth_info->key_info.cipher_suite = WLAN_CIPHER_SUITE_WEP104;
	}
	...
	if (req->auth_data_len) {
		...
		pr_err("%s: SAE_DATA_LEN =  %d\n",
		       __func__, (unsigned int)req->auth_data_len);
		pr_err("%s: SAE DATA DUMP:\n", __func__);

		print_hex_dump(KERN_DEBUG, "", DUMP_PREFIX_NONE, 16,
			       1, req->auth_data, (unsigned int)req->auth_data_len, 1);
...
void nrf_wifi_cfg80211_auth_resp_callbk_fn(void *os_vif_ctx,
					     ...
					     unsigned int event_len)
{
	...
	cfg80211_rx_mlme_mgmt(vif_ctx_lnx->netdev,
			      auth_resp_event->frame.frame,
			      auth_resp_event->frame.frame_len);

}


int nrf_wifi_cfg80211_assoc(struct wiphy *wiphy,
			      struct net_device *netdev,
			      struct cfg80211_assoc_request *req)
{
	...
	memcpy(assoc_info->nrf_wifi_bssid,
	       req->bss->bssid,
	       ETH_ALEN);
...
void nrf_wifi_cfg80211_assoc_resp_callbk_fn(void *os_vif_ctx,
					      ...
					      unsigned int event_len)
{
	...
	cfg80211_rx_assoc_resp(vif_ctx_lnx->netdev,
			       vif_ctx_lnx->bss,
			       assoc_resp_event->frame.frame,
			       assoc_resp_event->frame.frame_len,
			       -1,
			       NULL,
			       0);
...
int nrf_wifi_cfg80211_deauth(struct wiphy *wiphy,
			       struct net_device *netdev,
			       struct cfg80211_deauth_request *req)
{
	...
	status = nrf_wifi_fmac_deauth(rpu_ctx_lnx->rpu_ctx,
					vif_ctx_lnx->if_idx,
					deauth_info);
...
void nrf_wifi_cfg80211_deauth_callbk_fn(void *os_vif_ctx,
					  ...
					  unsigned int event_len)
{
	...
	vif_ctx_lnx = os_vif_ctx;
#ifdef notyet
	cfg80211_tx_mlme_mgmt(vif_ctx_lnx->netdev,
			      deauth_event->frame.frame,
			      deauth_event->frame.frame_len,
			      false);
#else
	cfg80211_tx_mlme_mgmt(vif_ctx_lnx->netdev,
			      deauth_event->frame.frame,
...
int nrf_wifi_cfg80211_disassoc(struct wiphy *wiphy,
				 struct net_device *netdev,
				 struct cfg80211_disassoc_request *req)
{
	...
	memcpy(disassoc_info->mac_addr,
	       req->bss->bssid,
	       ETH_ALEN);

	if (req->local_state_change)
		disassoc_info->nrf_wifi_flags |= NRF_WIFI_CMD_MLME_LOCAL_STATE_CHANGE;
...
void nrf_wifi_cfg80211_disassoc_callbk_fn(void *os_vif_ctx,
...
#ifdef notyet
	cfg80211_tx_mlme_mgmt(vif_ctx_lnx->netdev,
			      disassoc_event->frame.frame,
			      disassoc_event->frame.frame_len,
			      false);
#else
	cfg80211_tx_mlme_mgmt(vif_ctx_lnx->netdev,
			      disassoc_event->frame.frame,
...
int nrf_wifi_cfg80211_mgmt_tx(struct wiphy *wiphy,
				struct wireless_dev *wdev,
				...
				u64 *cookie)
{
	...
	if (params->len) {
		memcpy(mgmt_tx_info->frame.frame,
		       params->buf,
		       params->len);
	...
	mgmt_tx_info->freq_params.center_frequency1 = 2412;	/* same as freq above */
                                        ...
                                        bool reg2)
{
        ...
        list_for_each_entry(reg, &wdev->mgmt_registrations, list) {
                ...
                memcpy(frame_info->frame_match.frame_match,
                       reg->match,
                       reg->match_len);
...
void nrf_wifi_cfg80211_mgmt_rx_callbk_fn(void *os_vif_ctx,
...
void nrf_wifi_cfg80211_unprot_mlme_mgmt_rx_callbk_fn(void *os_vif_ctx,
...
void nrf_wifi_cfg80211_tx_status_callbk_fn(void *os_vif_ctx,
					...
					unsigned int event_len)
{
	...
	list_for_each_entry_safe(cookie_info,
				 tmp,
				 &rpu_ctx_lnx->cookie_list,
				 list) {	
	...
	ack_event = tx_status_event->nrf_wifi_flags & NRF_WIFI_EVENT_MLME_ACK ? true : false;

	cfg80211_mgmt_tx_status(vif_ctx_lnx->netdev->ieee80211_ptr,
				host_cookie,
				tx_status_event->frame.frame,
				tx_status_event->frame.frame_len,
				ack_event,
				GFP_ATOMIC);
}
				      ...
				      struct wireless_dev *wdev)
{
	...
	vif_ctx_lnx = netdev_priv(wdev->netdev);
	rpu_ctx_lnx = vif_ctx_lnx->rpu_ctx;
	status = nrf_wifi_fmac_p2p_dev_start(rpu_ctx_lnx->rpu_ctx,
					       vif_ctx_lnx->if_idx);

	if (status == NRF_WIFI_STATUS_FAIL) {
		pr_err("%s: nrf_wifi_fmac_start_p2p_dev failed\n", __func__);
		goto out;
	}
				      ...
				      struct wireless_dev *wdev)
{
	...
	status = nrf_wifi_fmac_p2p_dev_stop(rpu_ctx_lnx->rpu_ctx,
					      vif_ctx_lnx->if_idx);

	if (status == NRF_WIFI_STATUS_FAIL)
		pr_err("%s: nrf_wifi_fmac_stop_p2p_dev failed\n", __func__);
#endif
}
...
#ifdef notyet
	status = nrf_wifi_fmac_p2p_roc_start(rpu_ctx_lnx->rpu_ctx,
					       vif_ctx_lnx->if_idx,
					       roc_info);

	if (status == NRF_WIFI_STATUS_FAIL) {
		pr_err("%s: nrf_wifi_fmac_remain_on_channel failed\n", __func__);
		goto out;
	}
...
void nrf_wifi_cfg80211_roc_callbk_fn(void *os_vif_ctx,
				       ...
				       unsigned int event_len)
{
	...
	if (match_cookie_found) {
		...
		cfg80211_ready_on_channel(vif_ctx_lnx->wdev,
					  host_cookie,
					  chan,
					  duration,
					  GFP_KERNEL);
						 ...
						 u64 cookie)
{
	...
	status = nrf_wifi_fmac_p2p_roc_stop(rpu_ctx_lnx->rpu_ctx,
					      vif_ctx_lnx->if_idx,
					      cookie);

	if (status == NRF_WIFI_STATUS_FAIL) {
		pr_err("%s:  nrf_wifi_fmac_cancel_remain_on_channel failed\n", __func__);
		goto out;
	}
out:
	return status;
...
void nrf_wifi_cfg80211_roc_cancel_callbk_fn(void *os_vif_ctx,
...
int nrf_wifi_cfg80211_suspend(struct wiphy *wiphy,
				struct cfg80211_wowlan *wow)
{
	...
	if(wow) {
		if(wow->any)
			flag |= NRF_WIFI_SET_WOWLAN_FLAG_TRIG_ANY;

		if(wow->disconnect)
			flag |= NRF_WIFI_SET_WOWLAN_FLAG_TRIG_DISCONNECT;

		if(wow->magic_pkt)
			flag |= NRF_WIFI_SET_WOWLAN_FLAG_TRIG_MAGIC_PKT;

		if(wow->gtk_rekey_failure)
			flag |= NRF_WIFI_SET_WOWLAN_FLAG_TRIG_GTK_REKEY_FAILURE;

		if(wow->eap_identity_req)
			flag |= NRF_WIFI_SET_WOWLAN_FLAG_TRIG_EAP_IDENT_REQUEST;
	...
	status = nrf_wifi_fmac_set_wowlan(rpu_ctx_lnx->rpu_ctx, flag);

	if (status == NRF_WIFI_STATUS_FAIL) {
		pr_err("%s: nrf_wifi_fmac_set_wowlan failed\n", __func__);
		goto out;
	}

	status = nrf_wifi_fmac_suspend(rpu_ctx_lnx->rpu_ctx);
...
int nrf_wifi_cfg80211_resume(struct wiphy *wiphy)
{
	...
	status = nrf_wifi_fmac_resume(rpu_ctx_lnx->rpu_ctx);

	if (status == NRF_WIFI_STATUS_FAIL) {
		pr_err("%s:  nrf_wifi_fmac_resume failed\n", __func__);
		goto out;
	}
out:
	return status;
...
void nrf_wifi_cfg80211_set_wakeup(struct wiphy *wiphy,
				    bool enabled)
{
	struct nrf_wifi_ctx_lnx *rpu_ctx_lnx = NULL;
				    ...
				    struct cfg80211_qos_map *qos_map)
{
	...
	if (qos_map) {
		memcpy(&qos_map_info->qos_map_info,
		       qos_map,
		       sizeof(struct cfg80211_qos_map));
		qos_map_info->qos_map_info_len = sizeof(struct cfg80211_qos_map);
			  ...
			  struct station_info *sinfo)
{
	...
	if(sta->valid_fields & NRF_WIFI_STA_INFO_TX_BITRATE_VALID){
		...
		if (sta->tx_bitrate.nrf_wifi_flags & NRF_WIFI_RATE_INFO_40_MHZ_WIDTH)
			sinfo->txrate.bw = RATE_INFO_BW_40;
		else if (sta->tx_bitrate.nrf_wifi_flags & NRF_WIFI_RATE_INFO_80_MHZ_WIDTH)
			sinfo->txrate.bw = RATE_INFO_BW_80;
		else if (sta->tx_bitrate.nrf_wifi_flags & NRF_WIFI_RATE_INFO_160_MHZ_WIDTH)
			sinfo->txrate.bw = RATE_INFO_BW_160;
		else
			sinfo->txrate.bw = RATE_INFO_BW_20;

		if (sta->tx_bitrate.nrf_wifi_flags & NRF_WIFI_RATE_INFO_SHORT_GI)
			sinfo->txrate.flags |= RATE_INFO_FLAGS_SHORT_GI;

		if (sta->tx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_VALID)
			sinfo->txrate.legacy = sta->tx_bitrate.bitrate;

		if (sta->tx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_MCS_VALID){
		...
		if (sta->tx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_VHT_MCS_VALID){

			sinfo->txrate.mcs = sta->tx_bitrate.vht_mcs;
			sinfo->txrate.flags |= RATE_INFO_FLAGS_VHT_MCS;
		}

		if(sta->tx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_VHT_NSS_VALID)
	...
	if(sta->valid_fields & NRF_WIFI_STA_INFO_RX_BITRATE_VALID){
		...
		if (sta->rx_bitrate.nrf_wifi_flags & NRF_WIFI_RATE_INFO_40_MHZ_WIDTH)
			sinfo->rxrate.bw = RATE_INFO_BW_40;
		else if (sta->rx_bitrate.nrf_wifi_flags & NRF_WIFI_RATE_INFO_80_MHZ_WIDTH)
			sinfo->rxrate.bw = RATE_INFO_BW_80;
		else if (sta->rx_bitrate.nrf_wifi_flags & NRF_WIFI_RATE_INFO_160_MHZ_WIDTH)
			sinfo->rxrate.bw = RATE_INFO_BW_160;
		else
			sinfo->rxrate.bw = RATE_INFO_BW_20;

		if (sta->rx_bitrate.nrf_wifi_flags & NRF_WIFI_RATE_INFO_SHORT_GI)
			sinfo->rxrate.flags |= RATE_INFO_FLAGS_SHORT_GI;

		if (sta->rx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_VALID)
			sinfo->rxrate.legacy = sta->rx_bitrate.bitrate;

		if (sta->rx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_MCS_VALID) {

			sinfo->rxrate.mcs = sta->rx_bitrate.nrf_wifi_mcs;
			sinfo->rxrate.flags |= RATE_INFO_FLAGS_MCS;
		}

		if (sta->rx_bitrate.nrf_wifi_flags & NRF_WIFI_RATE_INFO_BITRATE_VHT_MCS_VALID) {
			sinfo->rxrate.mcs = sta->rx_bitrate.vht_mcs;
			sinfo->rxrate.flags |= RATE_INFO_FLAGS_VHT_MCS;
		}

		if (sta->rx_bitrate.nrf_wifi_flags & NRF_WIFI_RATE_INFO_BITRATE_VHT_NSS_VALID)
	...
	if(sta->valid_fields & NRF_WIFI_STA_INFO_STA_BSS_PARAMS_VALID){
		...
		sinfo->bss_param.beacon_interval = sta->bss_param.beacon_interval;
	...
	if(sta->valid_fields & NRF_WIFI_STA_INFO_CHAIN_SIGNAL_VALID){
		...
		memcpy(sinfo->chain_signal, sta->chain_signal, sizeof(sinfo->chain_signal));
	...
	if(sta->valid_fields & NRF_WIFI_STA_INFO_CHAIN_SIGNAL_AVG_VALID){
		...
		memcpy(sinfo->chain_signal_avg, sta->chain_signal_avg, sizeof(sinfo->chain_signal_avg));
...
int nrf_wifi_cfg80211_get_station(struct wiphy *wiphy,
				    ...
				    struct station_info *sinfo)
{
	...
	nrf_wifi_fmac_get_station(rpu_ctx_lnx->rpu_ctx,
				    vif_ctx_lnx->if_idx, (void*)mac);

	while(!vif_ctx_lnx->station_info && (count-- > 0)) msleep(100);
				     ...
				     int *dbm)
{
	...
	nrf_wifi_fmac_get_tx_power(rpu_ctx_lnx->rpu_ctx,
				   vif_ctx_lnx->if_idx);
				    ...
				    struct cfg80211_chan_def *chandef)
{
	...
	nrf_wifi_fmac_get_channel(rpu_ctx_lnx->rpu_ctx,
				  vif_ctx_lnx->if_idx);
	...
	chandef->chan = ieee80211_get_channel(wiphy, 
					      vif_ctx_lnx->chan_def->chan.center_frequency);
...
#endif
	.mgmt_frame_register =  nrf_wifi_cfg80211_mgmt_frame_reg,
...
#ifdef RPU_SUPPORT_WAPI
	WLAN_CIPHER_SUITE_SMS4
#endif
};

#ifdef RPU_VHT_SUPPORT
...
#ifdef notyet
	/* Advertise below support only if the RPU supports it */
...
struct wiphy *cfg80211_if_init(void)
{
	...
	wiphy = wiphy_new_nm(&cfg80211_ops,
			     sizeof(struct nrf_wifi_ctx_lnx),
			     NULL);
